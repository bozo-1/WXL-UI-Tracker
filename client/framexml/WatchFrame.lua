...
function WatchFrame_DisplayTrackedQuests (lineFrame, initialOffset, maxHeight, frameWidth)
	...
	for i = 1, numQuestWatches do
		...
		if ( questIndex ) then
			...
			-- check filters
			local filterOK = true;
			if ( not LOCAL_MAP_QUESTS[questID] ) then -- WXL: dropdown mirrors M list (eligibility, incl. completed)
				filterOK = false;
			end			
...
function WatchFrame_GetCurrentMapQuests()
	...
	-- WXL: the objective tracker iterates the WATCH list (GetNumQuestWatches),
	-- while the M tracker uses CURRENT_MAP_QUESTS - a quest can be in M but
	-- never watched, so no filter can surface it. Auto-watch every M-listed
	-- quest so the dropdown shows exactly the same quests as the M list
	-- (eligibility mirror). Quest-watch sync never affects the marker, which
	-- mirrors Blizzard's selection registers, not the watch list.
	for i = 1, GetNumQuestLogEntries() do
		local _, _, _, _, isHeader, _, _, _, questId = GetQuestLogTitle(i);
		if ( not isHeader and questId and CURRENT_MAP_QUESTS[questId] and not IsQuestWatched(i) ) then
			AddQuestWatch(i);
		end
	end
end
