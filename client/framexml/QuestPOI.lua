...
-- WXL: custom objective-tracker dropdown disc. Every dropdown button --
-- numbered (1,2,3..) AND completed (?) -- uses the same disc cell from
-- the native UI-QuestPoi-NumberIcons atlas, rendered at 1:1. The number
-- or ? glyph stays as-is (yellow). Scoped to WatchFrameLines; the
-- world-map (M) buttons keep vanilla art. NOTE: keep the literal 0.5 here
-- -- QUEST_POI_COLOR_YELLOW is a file-local declared below and is out of
-- lexical scope at this point.
function WX_ForceWatchPoiFace(poiButton)
	if ( poiButton and poiButton.parentName == "WatchFrameLines" ) then
		poiButton.normalTexture:SetTexCoord(0.500, 0.625, 0.875, 1.0);
		poiButton.pushedTexture:SetTexCoord(0.500, 0.625, 0.875, 1.0);
		poiButton.highlightTexture:SetTexCoord(0.500, 0.625, 0.875, 1.0);
		if ( poiButton.type == QUEST_POI_NUMERIC ) then
			QuestPOI_SetTextColor(poiButton, 0.5);
		end
	end
end
...
function QuestPOI_DisplayButton(parentName, buttonType, buttonIndex, questId)
	...
	WX_ForceWatchPoiFace(poiButton);
	poiButton:Show();
	return poiButton;
end
...
function QuestPOI_SelectButton(poiButton)
	if ( poiButton ) then
		...
		-- WXL: keep the dropdown disc + yellow number regardless of selection
		WX_ForceWatchPoiFace(poiButton);
	end
end
